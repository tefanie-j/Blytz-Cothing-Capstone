...
/* ==========================================================================
   PAGE 1 — DESIGN CREATION / PROMPT BUILDER
   ========================================================================== */
function initDesignPage() {
  const state = loadState();
  if (state.designStatus === 'submitted') {
    // Reset for a new design
    Object.assign(state, defaultState());
    saveState(state);
  }

  // ---- Garment Type ----
  const typeGrid = document.getElementById('garmentTypeGrid');
  typeGrid.innerHTML = GARMENT_TYPES.map(t => `
    <button type="button" class="type-card${state.garmentType === t.id ? ' selected' : ''}" data-type="${t.id}">
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M9 4l-5 2v4l3-1v11h10V9l3 1V6l-5-2-2 2h-2z"/></svg>
      <span>${t.label}</span>
    </button>`).join('');
  typeGrid.addEventListener('click', e => {
    const card = e.target.closest('.type-card');
    if (!card) return;
    state.garmentType = card.dataset.type;
    saveState(state);
    typeGrid.querySelectorAll('.type-card').forEach(c => c.classList.toggle('selected', c === card));
    renderGarmentPreview();
  });

  // ---- Fit ----
  const fitWrap = document.getElementById('fitOptions');
  fitWrap.innerHTML = FITS.map(f => `<button type="button" class="fit-btn${state.fit === f ? ' selected' : ''}" data-fit="${f}">${f}</button>`).join('');
  fitWrap.addEventListener('click', e => {
    const b = e.target.closest('.fit-btn');
    if (!b) return;
    state.fit = b.dataset.fit;
    saveState(state);
    fitWrap.querySelectorAll('.fit-btn').forEach(x => x.classList.toggle('selected', x === b));
    renderGarmentPreview();
  });

  // ---- Color ----
  const colorGrid = document.getElementById('colorGrid');
  colorGrid.innerHTML = COLORS.map(c => `<button type="button" class="color-swatch${state.color === c.hex ? ' selected' : ''}" style="background:${c.hex};" data-hex="${c.hex}" data-name="${c.name}" aria-label="${c.name}"></button>`).join('')
    + `<label class="color-custom" aria-label="Custom color" title="Custom color"><input type="color" id="customColorInput" value="${state.color}"></label>`;
  colorGrid.addEventListener('click', e => {
    const sw = e.target.closest('.color-swatch');
    if (!sw) return;
    state.color = sw.dataset.hex; state.colorName = sw.dataset.name;
    saveState(state);
    colorGrid.querySelectorAll('.color-swatch').forEach(s => s.classList.toggle('selected', s === sw));
    renderGarmentPreview();
  });
  document.getElementById('customColorInput').addEventListener('input', e => {
    state.color = e.target.value; state.colorName = 'Custom';
    saveState(state);
  ...
  sideToggle.addEventListener('click', e => {
    ...
    if (!b) return;
    state.currentSide = b.dataset.side;
    saveState(state);
    sideToggle.querySelectorAll('button').forEach(x => x.classList.toggle('active', x === b));
    renderGarmentPreview();
  });
  sideToggle.querySelectorAll('button').forEach(b => b.classList.toggle('active', b.dataset.side === state.currentSide));

  // ---- Mode toggle ----
  const modeToggle = document.getElementById('modeToggle');
  modeToggle.querySelectorAll('button').forEach(b => b.classList.toggle('active', b.dataset.mode === state.promptMode));
  modeToggle.addEventListener('click', e => {
    const b = e.target.closest('button[data-mode]');
    if (!b || b.dataset.mode === state.promptMode) return;
    // Check if there are existing prompts
    const hasPrompts = hasAnyPrompts(state);
    if (hasPrompts) {
      pendingModeSwitch = b.dataset.mode;
      document.getElementById('modeSwitchModal').classList.add('is-open');
    } else {
      applyModeSwitch(b.dataset.mode);
    }
  });

  let pendingModeSwitch = null;
  document.getElementById('modeSwitchCancel').addEventListener('click', () => {
    pendingModeSwitch = null;
    document.getElementById('modeSwitchModal').classList.remove('is-open');
  });
  document.getElementById('modeSwitchConfirm').addEventListener('click', () => {
  ...
  function applyModeSwitch(mode) {
    ...
    state.overall = { front: null, back: null, frontBack: null };
    state.individual = { front: [], back: [] };
    saveState(state);
    modeToggle.querySelectorAll('button').forEach(b => b.classList.toggle('active', b.dataset.mode === mode));
    document.getElementById('panelOverall').style.display = mode === 'overall' ? 'block' : 'none';
    document.getElementById('panelIndividual').style.display = mode === 'individual' ? 'block' : 'none';
    refreshOverallCards();
    refreshAreaButtons();
    renderPromptStack();
  }

  document.getElementById('panelOverall').style.display = state.promptMode === 'overall' ? 'block' : 'none';
  document.getElementById('panelIndividual').style.display = state.promptMode === 'individual' ? 'block' : 'none';

  // ---- Overall option cards ----
  let activeOverallKey = null;
  const overallEditor = document.getElementById('overallEditor');
  const overallEditorLabel = document.getElementById('overallEditorLabel');
  const overallPromptText = document.getElementById('overallPromptText');

  document.getElementById('overallOptionGrid').addEventListener('click', e => {
    const card = e.target.closest('.overall-option-card');
    if (!card) return;
    const key = card.dataset.overall; // 'front' | 'back' | 'frontBack'

    // Conflict check
    const conflict = getOverallConflict(state, key);
    if (conflict) {
      document.getElementById('conflictMsg').textContent = conflict.msg;
      document.getElementById('conflictModal').classList.add('is-open');
      document.getElementById('conflictCancel').onclick = () => document.getElementById('conflictModal').classList.remove('is-open');
      document.getElementById('conflictReplace').onclick = () => {
        document.getElementById('conflictModal').classList.remove('is-open');
        conflict.clear(state);
        saveState(state);
        refreshOverallCards();
        openOverallEditor(key);
      };
      return;
    }

    // If already has a prompt, open editor pre-filled for editing
    openOverallEditor(key);
  });

  function openOverallEditor(key) {
    activeOverallKey = key;
    overallEditorLabel.textContent = key === 'front' ? 'Overall Front' : key === 'back' ? 'Overall Back' : 'Overall Front & Back';
    overallPromptText.value = state.overall[key] || '';
    overallPromptText.placeholder = key === 'frontBack'
      ? 'Describe the complete design for both front and back...'
      : `Describe the complete ${key} design here...`;
    document.getElementById('overallOptionGrid').querySelectorAll('.overall-option-card').forEach(c => c.classList.toggle('active', c.dataset.overall === key));
    overallEditor.style.display = 'block';
    overallPromptText.focus();
  }

  document.getElementById('overallCancelBtn').addEventListener('click', () => {
    overallEditor.style.display = 'none';
    document.getElementById('overallOptionGrid').querySelectorAll('.overall-option-card').forEach(c => c.classList.remove('active'));
    activeOverallKey = null;
  });

  document.getElementById('overallAddBtn').addEventListener('click', () => {
    const text = overallPromptText.value.trim();
    if (!text) { showToast('Enter a design description first.', 'error'); return; }
    state.overall[activeOverallKey] = text;
    saveState(state);
    overallEditor.style.display = 'none';
    activeOverallKey = null;
    document.getElementById('overallOptionGrid').querySelectorAll('.overall-option-card').forEach(c => c.classList.remove('active'));
  ...
  function refreshOverallCards() {
    document.querySelectorAll('.overall-option-card').forEach(card => {
      const key = card.dataset.overall;
      card.classList.toggle('filled', Boolean(state.overall[key]));
    });
  }
  refreshOverallCards();

  // ---- Individual areas ----
  let individualActiveSide = 'front';
  let individualActiveArea = null;
  const individualEditor = document.getElementById('individualEditor');
  const individualEditorLabel = document.getElementById('individualEditorLabel');
  const individualPromptText = document.getElementById('individualPromptText');

  document.getElementById('areaSideTabs').addEventListener('click', e => {
    const b = e.target.closest('button[data-side]');
    if (!b) return;
    individualActiveSide = b.dataset.side;
    individualActiveArea = null;
    individualEditor.style.display = 'none';
    document.getElementById('areaSideTabs').querySelectorAll('button').forEach(x => x.classList.toggle('active', x === b));
    refreshAreaButtons();
  });

  function refreshAreaButtons() {
    const areas = individualActiveSide === 'front' ? FRONT_AREAS : BACK_AREAS;
    const existing = state.individual[individualActiveSide];
    const grid = document.getElementById('areaBtnGrid');
    grid.innerHTML = areas.map(a => {
      const hasFill = existing.some(p => p.area === a.id);
      return `<button type="button" class="area-btn${hasFill ? ' filled' : ''}${individualActiveArea === a.id ? ' active' : ''}" data-area="${a.id}" data-label="${a.label}">${a.label}</button>`;
    }).join('');
    grid.querySelectorAll('.area-btn').forEach(b => {
      b.addEventListener('click', () => {
        individualActiveArea = b.dataset.area;
        const label = b.dataset.label;
        individualEditorLabel.textContent = `${individualActiveSide === 'front' ? 'Front' : 'Back'} — ${label}`;
        const existing = state.individual[individualActiveSide].find(p => p.area === individualActiveArea);
        individualPromptText.value = existing ? existing.prompt : '';
        individualPromptText.placeholder = `Describe the design for ${label.toLowerCase()}...`;
        individualEditor.style.display = 'block';
        individualPromptText.focus();
        grid.querySelectorAll('.area-btn').forEach(x => x.classList.toggle('active', x === b));
      });
    });
  }
  refreshAreaButtons();

  document.getElementById('individualCancelBtn').addEventListener('click', () => {
    individualEditor.style.display = 'none';
    individualActiveArea = null;
    document.getElementById('areaBtnGrid').querySelectorAll('.area-btn').forEach(b => b.classList.remove('active'));
  });

  document.getElementById('individualAddBtn').addEventListener('click', () => {
    const text = individualPromptText.value.trim();
    if (!text) { showToast('Enter a description for this area first.', 'error'); return; }
    const areas = individualActiveSide === 'front' ? FRONT_AREAS : BACK_AREAS;
    const areaObj = areas.find(a => a.id === individualActiveArea);
    const arr = state.individual[individualActiveSide];
    const idx = arr.findIndex(p => p.area === individualActiveArea);
    const entry = { id: idx >= 0 ? arr[idx].id : uid(), side: individualActiveSide, area: individualActiveArea, label: areaObj.label, prompt: text };
  ...
  document.getElementById('editPromptSave').addEventListener('click', () => {
    const text = document.getElementById('editPromptText').value.trim();
    if (!text) return;
    if (editingPromptId && editingPromptId.startsWith('overall_')) {
      const key = editingPromptId.replace('overall_', '');
      state.overall[key] = text;
    } else {
      ['front', 'back'].forEach(side => {
        const idx = state.individual[side].findIndex(p => p.id === editingPromptId);
        if (idx >= 0) state.individual[side][idx].prompt = text;
      });
    }
    saveState(state);
    document.getElementById('editPromptModal').classList.remove('is-open');
    refreshOverallCards();
    renderPromptStack();
  });

  function openEditModal(id, label, text) {
    editingPromptId = id;
    document.getElementById('editPromptLabel').textContent = label;
    document.getElementById('editPromptText').value = text;
    document.getElementById('editPromptModal').classList.add('is-open');
  }

  // ---- Prompt stack ----
  function renderPromptStack() {
    const wrap = document.getElementById('promptStack');
    if (!wrap) return;
    let html = '';
    if (state.promptMode === 'overall') {
      const groups = [
        { key: 'front', label: 'FRONT', heading: 'OVERALL FRONT' },
        { key: 'back', label: 'BACK', heading: 'OVERALL BACK' },
        { key: 'frontBack', label: 'FRONT & BACK', heading: 'OVERALL FRONT & BACK' }
      ];
      groups.forEach(g => {
        if (!state.overall[g.key]) return;
        html += `<div class="prompt-stack-group">
          <div class="prompt-stack-heading">${g.label}</div>
          <div class="prompt-card">
            <div class="pc-label">${g.heading}</div>
            <div class="pc-text">"${esc(state.overall[g.key])}"</div>
            <div class="pc-actions">
              <button onclick="window._editOverall('${g.key}')">Edit</button>
              <button class="danger" onclick="window._deleteOverall('${g.key}')">Delete</button>
            </div>
          </div>
        </div>`;
      });
      ...
      ['front', 'back'].forEach(side => {
        const arr = state.individual[side];
        html += `<div class="prompt-stack-group">
          <div class="prompt-stack-heading">${side.toUpperCase()}</div>`;
        if (!arr.length) {
          html += `<p class="prompt-empty-note">No prompts added for the ${side.charAt(0).toUpperCase() + side.slice(1)}.</p>`;
        } else {
          arr.forEach(p => {
            html += `<div class="prompt-card">
              <div class="pc-label">${side.toUpperCase()} — ${esc(p.label)}</div>
              <div class="pc-text">"${esc(p.prompt)}"</div>
              <div class="pc-actions">
                <button onclick="window._editIndividual('${p.id}')">Edit</button>
                <button class="danger" onclick="window._deleteIndividual('${p.id}','${side}')">Delete</button>
              </div>
            </div>`;
          });
        }
    ...
    if (!html) html = '<p class="prompt-empty-note" style="color:rgba(246,244,239,0.4);font-size:13px;">No design prompts added yet.</p>';
    wrap.innerHTML = html;
  }

  // Expose handlers for inline onclick (safe in prototype context)
  window._editOverall = (key) => {
    const labels = { front: 'Overall Front', back: 'Overall Back', frontBack: 'Overall Front & Back' };
    openEditModal('overall_' + key, labels[key], state.overall[key]);
  };
  window._deleteOverall = (key) => {
    state.overall[key] = null;
    saveState(state);
    refreshOverallCards();
    renderPromptStack();
  };
  window._editIndividual = (id) => {
    let found;
    ['front', 'back'].forEach(s => { const p = state.individual[s].find(x => x.id === id); if (p) found = p; });
    if (found) openEditModal(id, `${found.side.toUpperCase()} — ${found.label}`, found.prompt);
  };
  window._deleteIndividual = (id, side) => {
    state.individual[side] = state.individual[side].filter(p => p.id !== id);
    saveState(state);
    refreshAreaButtons();
    renderPromptStack();
  };
  ...
  document.getElementById('continueBtn').addEventListener('click', () => {
    if (!state.garmentType) { showToast('Choose a garment type first.', 'error'); return; }
    if (!state.fit) { showToast('Choose a fit (Female, Male, Unisex, or Kids).', 'error'); return; }
    if (!state.color) { showToast('Choose a garment color.', 'error'); return; }
    if (!hasAnyPrompts(state)) { showToast('Add at least one design prompt before continuing.', 'error'); return; }

    // FRONTEND SAMPLE ONLY
    // ============================================================
    // TODO: In the real system, this would send the design prompts
    // to an AI generation service and receive back generated design
    // objects. For this prototype, we seed sample objects instead.
    // ============================================================
  ...
  function renderGarmentPreview() {
    ...
    const mock = document.getElementById('garmentMock');
    if (!state.garmentType || !state.fit || !state.color) {
    ...
    mock.innerHTML = garmentSVG(state.garmentType, state.color);
  }
}
